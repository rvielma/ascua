...
export function Panel(props: { sesion: () => Sesion; onsalir: () => void; pedidos: Pedido[] }) {
  ...
  /** `/pedidos` — la tabla, con su filtro y su orden. */
  const Pedidos = () => view`
    <div class="rejilla">
  ...
  const panel = view`
    <div class="panel">
      <header class="barra">
        <strong>Ascua</strong>
        <nav>
          <a href="/" class=${() => (seccion() === "resumen" ? "activa" : "")}>Resumen</a>
          <a href="/pedidos" class=${() => (seccion() === "pedidos" ? "activa" : "")}>Pedidos</a>
        </nav>
        <div class="sesion">
      ...
      <style>
        ...
        nav a { font-size: .88rem; padding: .35rem .7rem; border-radius: 7px;
                color: var(--tenue); text-decoration: none; }
        nav a.activa { background: var(--fondo); color: inherit; font-weight: 600; }
